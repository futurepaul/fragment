...
fetch reaches its upstream once). A run in flight across the deploy that
began keeping answers finds none kept: it starts again by itself as its
next attempt, once (`run.resumed` in the events), as a replay would; a
run whose answers are missing otherwise is held. A step that fails for a reason that may
