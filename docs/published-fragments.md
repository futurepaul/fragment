...
| Dynamic `app.mjs` + `applib/` from the live pin | meatproxy, strategy-vault, events-rfc, sycamore | e2e `app`, `static-root`, `build` |
...
| Secrets: declared by name, wrapped at rest, injected into runs | meatproxy, events-rfc, sycamore | e2e `auth`, `lockdown`, `workflows`; `runtime/test/secretwrap.test.mjs` |
| Platform AI: `fragment:ai` text, image, video | meatproxy | e2e `gen`, `cron` (moves from fal to OpenRouter) |
| Visibility: `public`, `link` (view token → per-fragment cookie), `viewers` | all | e2e `platform`, `lockdown`, `auth` |
| Deploy, preview, rollback, drafts (git `main`/`live`) | all | e2e `deploy` |
| Folder sync: conflict copies, mass-delete guard, journals bound to repo identity | all (authoring) | e2e `filesync`, `cli-lane`, `converge`; CLI unit tests |
