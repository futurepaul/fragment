...
|---|---|---|
| `src/agent.js` | AgentCell: libfx turn loop, durable turns accepted before work, per-conversation ordering with retry backoff (5 s × 4^attempt, 4 attempts), lease + watchdog runner, operation log with `effectKey` dedupe, notes injected into the next turn, workspace progress cards | phase 5 takes the behaviors (ordering, retries, progress cards, notes), not the code: the loop is goose's, from `spike/goose-agent` (ROADMAP decision 10) |
| `src/inference.js` | libfx speaks the Vercel AI Gateway protocol; this adapts it to an OpenAI-compatible endpoint and lifts tool-result images into a user message (`liftToolImages`, `KEEP_IMAGE_STEPS = 2`) | not ported: goose talks to OpenRouter directly; keep the image-lifting rule in mind for tool screenshots |
...
| `src/workspace.js`, `gateway/workspaces.mjs`, `workspace/kit/*` | Sprite builder workspaces: versioned kit, bootstrap, ACP runner (`acp-job.mjs`), event log read by byte offset, cancel file → `session/cancel`, tree reset, one job per app, watchdog alarm | phase 8 keeps the ideas (one job per app, tree reset, watchdog); the fx/ACP runner is replaced by `fragment computer serve` (goose) |
| `upstream/desktop/*` | Finite-styled chat and desktop UI (tokens from finitecomputer-v2 `ocean-shell.css`; transcript with tool rollups, streaming, live build cards, a DOM-only markdown renderer), plus the Desktop UI session's layout on branch `claude/desktop-ui` | phase 6 (the desktop) |
...
| `computer/*` | the Substrate pet computer (Chromium, Xvfb, Openbox, `control.py` with screenshot/open/click/type/key/shell and op-idempotency records) | phase 8: the same pieces as Sprite services; Substrate itself is dropped |
