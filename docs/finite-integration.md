...
| Model access and metering | Finite Private + usage accounting (FIN-13) | **built (slice C) for `job.ai`:** each org's own OpenRouter key with the allowance as its monthly limit; every paid step reserves, then settles to OpenRouter's reported cost; each usage row records FIN-10's report fields (source reference, agent, billing org, period, unit, quantity), once per step. Agents' own turns are not metered yet (debt ledger) | model calls move to Finite Private with Core's authorization; the same rows go to Core |
