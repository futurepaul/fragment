...
| Model access and metering | Finite Private + usage accounting (FIN-13) | **built (slice C) for `job.ai`, and for agents' model calls since the agent add-on:** each org's own OpenRouter key with the allowance as its monthly limit; every paid step reserves, then settles to OpenRouter's reported cost; each usage row records FIN-10's report fields (source reference, agent, billing org, period, unit, quantity), once per step. An agent's model calls do the same on its owner's month, one `agent.text` row per call (`POST /api/budget/reserve`, `/settle`) | model calls move to Finite Private with Core's authorization; the same rows go to Core |
