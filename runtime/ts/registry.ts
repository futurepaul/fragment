// Fragment lifecycle + the _registry cell's routes. Only the _registry
// instance serves registryRoute; initCell runs inside each new fragment.
...
export async function registryRoute(cell, request, url) {
  ...
  if (p === "/__registry/roles-sync" && request.method === "POST") {
    ...
    const add = (npub, role) => {
      try { cell.sql.exec("INSERT INTO roles (name, pubkey, role) VALUES (?, ?, ?) ON CONFLICT DO NOTHING", name, hexFromNpub(npub), role); } catch {}
    };
    try { cell.sql.exec("INSERT INTO roles (name, pubkey, role) VALUES (?, ?, 'owner') ON CONFLICT DO NOTHING", name, owner); } catch {}
...
export async function syncRolesToRegistry(cell) {
