...
const ManifestSchema = Type.Object({
  ...
  visibility: Type.Union([Type.Literal("public"), Type.Literal("viewers"), Type.Literal("link"), Type.Literal("token")]), // token = legacy, migrates
...
export function normalizeManifest(m) {
  ...
  // v0.5.0 hard cut migration: the old "token" literal reads as "link"
  // (pre-rename manifests would otherwise 401 even with a valid ?view=)
