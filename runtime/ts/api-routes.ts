...
export async function apiRoute(cell, request, url) {
  ...
  if (p === "/storage-token" && request.method === "GET") {
    ...
    // must fail here, not at code.storage. Manual base64url→padded-base64:
    // jose emits UNPADDED base64url and BOTH raw atob and jose's decoder
    // choke on it inside workerd (live-found via strategy-vault; the same
    // tokens verify fine at code.storage — only our decode was broken)
