...
function genOpts(kind, opts) {
  const out = {};
  if (!opts || typeof opts !== "object") return out;
  const int = (v, lo, hi) => Number.isInteger(v) && v >= lo && v <= hi ? v : void 0;
  if (kind === "image") {
    ...
    const n = int(opts.num_images, 1, 4);
    if (n !== void 0) out.num_images = n;
    const steps = int(opts.num_inference_steps, 1, 50);
    if (steps !== void 0) out.num_inference_steps = steps;
    ...
    const seed = int(opts.seed, 0, Number.MAX_SAFE_INTEGER);
    if (seed !== void 0) out.seed = seed;
  } else {
    const dur = int(opts.duration, 1, 15);
    if (dur !== void 0) out.duration = dur;
...
async function internalRoute(cell, request, url) {
  ...
  if (rest === "gen/start" && request.method === "POST") {
    ...
    if (typeof prompt !== "string" || !prompt.trim() || prompt.length > 4e3)
    ...
    const model = modelOverride || (kind === "image" ? cell.env.FRAGMENT_IMAGE_MODEL : cell.env.FRAGMENT_VIDEO_MODEL) || def.model;
    ...
    const path = `${dir}/${(/* @__PURE__ */ new Date()).toISOString().replace(/[-:T]/g, "").slice(0, 14)}-${randHex(4)}.${ext}`;
    if (path.startsWith("site/")) {
  ...
  if (rest === "gen/status" && request.method === "POST") {
    ...
    const valid = job && typeof job === "object" && (job.kind === "image" || job.kind === "video") && typeof job.model === "string" && /^[A-Za-z0-9][-A-Za-z0-9._/]{0,119}$/.test(job.model) && typeof job.requestId === "string" && job.requestId.length <= 128 && typeof job.path === "string" && job.path.length <= 200 && !job.path.includes("..") && !job.path.startsWith("/");
    if (!valid) return json({ error: "body: {job} \u2014 the opaque job object from gen/start" }, 400);
    const statusUrl = falUrlOk(cell.env, job.statusUrl) ? job.statusUrl : `${falBase(cell.env)}/${job.model}/requests/${encodeURIComponent(job.requestId)}/status`;
    const responseUrl = falUrlOk(cell.env, job.responseUrl) ? job.responseUrl : `${falBase(cell.env)}/${job.model}/requests/${encodeURIComponent(job.requestId)}`;
    let st;
    try {
