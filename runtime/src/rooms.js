...
function presenceList(cell, room) {
  const out = [];
...
function broadcast(cell, room, text) {
  for (const ws of cell.state.getWebSockets()) {
    try {
      const a = ws.deserializeAttachment();
...
async function webSocketMessage(cell, ws, raw) {
  let a;
  try {
    ...
    if (pin && row) {
      try {
        ...
        const ep = await cell.loadCode(
          ...
          ROOMS_MAIN,
          { "rooms.mjs": src },
          { kind: "draft", worker: "rooms", slug: pin, blessed: true }
        );
...
async function webSocketClose(cell, ws) {
  let a;
  try {
    a = ws.deserializeAttachment();
  } catch {
